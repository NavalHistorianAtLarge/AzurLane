...
var spine = (() => {
  ...
  // spine-pixi/src/require-shim.ts
    ...
    updateWorldTransformWith(x, y, rotation, scaleX, scaleY, shearX, shearY) {
      ...
      switch (this.data.transformMode) {
        ...
        case 4 /* NoScaleOrReflection */: {
          ...
          if (this.data.transformMode == 3 /* NoScale */ && pa * pd - pb * pc < 0 != (this.skeleton.scaleX < 0 != this.skeleton.scaleY < 0)) s = -s;
  ...
  // spine-pixi/src/SpineTexture.ts
  ...
  // spine-pixi/src/SlotMesh.ts
  var import_mesh = __require("@pixi/mesh");
  var import_core2 = __require("@pixi/core");
  var SlotMesh = class _SlotMesh extends import_mesh.Mesh {
    name = "";
    static auxColor = [0, 0, 0, 0];
    warnedTwoTint = false;
    constructor() {
      const geometry = new import_mesh.MeshGeometry();
      geometry.getBuffer("aVertexPosition").static = false;
      geometry.getBuffer("aTextureCoord").static = false;
      const meshMaterial = new import_mesh.MeshMaterial(import_core2.Texture.EMPTY);
  ...
  var Spine = class _Spine extends import_display.Container {
    ...
    set autoUpdate(value) {
      if (value) {
        import_core3.Ticker.shared.add(this.internalUpdate, this);
        this.autoUpdateWarned = false;
      } else {
        import_core3.Ticker.shared.remove(this.internalUpdate, this);
      }
    ...
    darkColor = new Color();
    constructor(skeletonData, options) {
      super();
      this.skeleton = new Skeleton(skeletonData);
      const animData = new AnimationStateData(skeletonData);
      this.state = new AnimationState(animData);
      this.autoUpdate = options?.autoUpdate ?? true;
      this.slotMeshFactory = options?.slotMeshFactory ?? (() => new SlotMesh());
    ...
    internalUpdate(_deltaFrame, deltaSeconds) {
      this.state.update(deltaSeconds ?? import_core3.Ticker.shared.deltaMS / 1e3);
    }
    updateTransform() {
    ...
    updateGeometry() {
      ...
      for (let i = 0, n = drawOrder.length; i < n; i++) {
        ...
        const vertexSize = _Spine.clipper.isClipping() ? 2 : useDarkColor ? _Spine.DARK_VERTEX_SIZE : _Spine.VERTEX_SIZE;
        if (!slot.bone.active) {
          _Spine.clipper.clipEndWithSlot(slot);
          continue;
        }
        ...
        if (attachment instanceof RegionAttachment) {
          ...
          _Spine.clipper.clipStart(slot, attachment);
          continue;
        } else {
          _Spine.clipper.clipEndWithSlot(slot);
          continue;
        ...
        if (texture != null) {
          ...
          if (_Spine.clipper.isClipping()) {
            _Spine.clipper.clipTriangles(this.verticesCache, numFloats, triangles, triangles.length, uvs, this.lightColor, this.darkColor, useDarkColor);
    ...
    setBonePosition(bone, position) {
      ...
      if (bone.parent) {
        const aux = bone.parent.worldToLocal(_Spine.vectorAux);
        bone.x = aux.x;
        bone.y = aux.y;
      } else {
        bone.x = _Spine.vectorAux.x;
        bone.y = _Spine.vectorAux.y;
      }
    }
    ...
    static from(skeletonAssetName, atlasAssetName, options) {
      const cacheKey = `${skeletonAssetName}-${atlasAssetName}-${options?.scale ?? 1}`;
      let skeletonData = _Spine.skeletonCache[cacheKey];
      if (skeletonData) {
        return new _Spine(skeletonData, options);
      }
      const skeletonAsset = import_assets.Assets.get(skeletonAssetName);
      const atlasAsset = import_assets.Assets.get(atlasAssetName);
  ...
  // spine-pixi/src/SpineDebugRenderer.ts
  ...
  var SpineDebugRenderer = class {
    ...
    registerSpine(spine) {
      ...
      const debugDisplayObjects = {
        ...
        skeletonXY: new import_graphics.Graphics(),
        regionAttachmentsShape: new import_graphics.Graphics(),
        meshTrianglesLine: new import_graphics.Graphics(),
        meshHullLine: new import_graphics.Graphics(),
        clippingPolygon: new import_graphics.Graphics(),
        boundingBoxesRect: new import_graphics.Graphics(),
        boundingBoxesCircle: new import_graphics.Graphics(),
        boundingBoxesPolygon: new import_graphics.Graphics(),
        pathsCurve: new import_graphics.Graphics(),
        pathsLine: new import_graphics.Graphics(),
    ...
    drawBonesFunc(spine, debugDisplayObjects, lineWidth, scale) {
      ...
      for (let i = 0, len = bones.length; i < len; i++) {
        ...
        const gp = new import_graphics.Graphics();
  ...
  // spine-pixi/src/darkTintMesh/DarkTintMesh.ts
  var import_mesh2 = __require("@pixi/mesh");

  // spine-pixi/src/darkTintMesh/DarkTintGeom.ts
  ...
  // spine-pixi/src/darkTintMesh/DarkTintMaterial.ts
...
`;
  var DarkTintMaterial = class extends import_core5.Shader {
    ...
    update() {
      if (this._colorDirty) {
        ...
        import_core5.Color.shared.setValue(this._tintColor).premultiply(this._alpha, applyToChannels).toArray(this.uniforms.uColor);
        import_core5.Color.shared.setValue(this._darkTintColor).premultiply(this._alpha, applyToChannels).toArray(this.uniforms.uDarkColor);
      }
  ...
  // spine-pixi/src/darkTintMesh/DarkTintMesh.ts
  var DarkTintMesh = class extends import_mesh2.Mesh {
    // eslint-disable-next-line @typescript-eslint/naming-convention
    _darkTintRGB = 0;
    constructor(texture) {
  ...
  // spine-pixi/src/DarkSlotMesh.ts
  ...
  // spine-pixi/src/assets/atlasLoader.ts
  ...
  var spineTextureAtlasLoader = {
    extension: import_core6.ExtensionType.Asset,
    loader: {
      extension: {
        type: import_core6.ExtensionType.LoadParser,
        priority: import_assets3.LoaderParserPriority.Normal,
        name: "spineTextureAtlasLoader"
      },
      test(url) {
        return (0, import_assets3.checkExtension)(url, ".atlas");
      },
      async load(url) {
        const response = await import_core6.settings.ADAPTER.fetch(url);
        const txt = await response.text();
        return txt;
      },
      testParse(asset, options) {
        const isExtensionRight = (0, import_assets3.checkExtension)(options.src, ".atlas");
      ...
      async parse(asset, options, loader) {
        ...
        let basePath = import_core6.utils.path.dirname(options.src);
        if (basePath && basePath.lastIndexOf("/") !== basePath.length - 1) {
          basePath += "/";
        }
        const retval = new TextureAtlas(asset);
        if (metadata.images instanceof import_core6.BaseTexture || typeof metadata.images === "string") {
        ...
        import_assets2.Assets.setPreferences({ preferCreateImageBitmap: false });
        for (const page of retval.pages) {
          const pageName = page.name;
          const providedPage = metadata?.images ? metadata.images[pageName] : void 0;
          if (providedPage instanceof import_core6.BaseTexture) {
            page.setTexture(SpineTexture.from(providedPage));
          } else {
            const url = providedPage ?? import_core6.utils.path.normalize([...basePath.split(import_core6.utils.path.sep), pageName].join(import_core6.utils.path.sep));
            const assetsToLoadIn = { src: url, data: { ...metadata.imageMetadata, ...{ alphaMode: page.pma ? import_core6.ALPHA_MODES.PMA : import_core6.ALPHA_MODES.UNPACK } } };
        ...
        import_assets2.Assets.setPreferences({ preferCreateImageBitmap: oldPreferCreateImageBitmap });
        return retval;
      }
    }
  };
  import_core6.extensions.add(spineTextureAtlasLoader);

  // spine-pixi/src/assets/skeletonLoader.ts
  var import_assets4 = __require("@pixi/assets");
  ...
  var spineLoaderExtension = {
    extension: import_core7.ExtensionType.Asset,
    loader: {
      extension: {
        type: import_core7.ExtensionType.LoadParser,
        priority: import_assets4.LoaderParserPriority.Normal
      },
      test(url) {
        return (0, import_assets4.checkExtension)(url, ".skel");
      },
      async load(url) {
        const response = await import_core7.settings.ADAPTER.fetch(url);
        const buffer = new Uint8Array(await response.arrayBuffer());
        return buffer;
      },
      testParse(asset, options) {
        const isJsonSpineModel = (0, import_assets4.checkExtension)(options.src, ".json") && isJson(asset);
        const isBinarySpineModel = (0, import_assets4.checkExtension)(options.src, ".skel") && isBuffer(asset);
        return Promise.resolve(isJsonSpineModel || isBinarySpineModel);
      }
    }
  };
  import_core7.extensions.add(spineLoaderExtension);

  // spine-pixi/src/darkTintMesh/DarkTintBatchGeom.ts
  var import_core8 = __require("@pixi/core");
  var DarkTintBatchGeometry = class extends import_core8.Geometry {
    // eslint-disable-next-line @typescript-eslint/naming-convention
    _buffer;
    // eslint-disable-next-line @typescript-eslint/naming-convention
    _indexBuffer;
    ...
    constructor(_static = false) {
      super();
      this._buffer = new import_core8.Buffer(void 0, _static, false);
      this._indexBuffer = new import_core8.Buffer(void 0, _static, true);
      this.addAttribute("aVertexPosition", this._buffer, 2, false, import_core8.TYPES.FLOAT).addAttribute("aTextureCoord", this._buffer, 2, false, import_core8.TYPES.FLOAT).addAttribute("aColor", this._buffer, 4, true, import_core8.TYPES.UNSIGNED_BYTE).addAttribute("aDarkColor", this._buffer, 4, true, import_core8.TYPES.UNSIGNED_BYTE).addAttribute("aTextureId", this._buffer, 1, true, import_core8.TYPES.FLOAT).addIndex(this._indexBuffer);
    }
  };

  // spine-pixi/src/darkTintMesh/DarkTintRenderer.ts
...
`;
  var DarkTintRenderer = class extends import_core9.BatchRenderer {
    static extension = {
      name: "darkTintBatch",
      type: import_core9.ExtensionType.RendererPlugin
    };
    constructor(renderer) {
      super(renderer);
      this.shaderGenerator = new import_core9.BatchShaderGenerator(vertex2, fragment2);
    ...
    packInterleavedGeometry(element, attributeBuffer, indexBuffer, aIndex, iIndex) {
      ...
      const argb = import_core9.Color.shared.setValue(element._tintRGB).toPremultiplied(alpha, (element._texture.baseTexture.alphaMode ?? 0) > 0);
      const darkargb = import_core9.Color.shared.setValue(element._darkTintRGB).toPremultiplied(alpha, (element._texture.baseTexture.alphaMode ?? 0) > 0);
